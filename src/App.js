...
function App() {
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      const response = await fetch('http://localhost:3003/generate-prompts', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className='app'>
      ...
      {submitMessage && (
      <section>
        <p className='select-message'>Select a suitable prompt to generate an image</p>
      </section>
      )}
      ...
      <ul>
        {prompts?.map((prompt, index) => (
          <button className={selectedPrompt===prompts[index]? 'highlighted':''}
          onClick={()=>getImages(prompt)}><li key={index}>{prompt}</li></button>
        ))}
      </ul>
      </section>
